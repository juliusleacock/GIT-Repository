<?php
	...
	function checkIfUserHasAccessToAHSLModule($users_id){
		...
		if(isset($users_id)){
			...
			$query_string = "SELECT COUNT(*) FROM ".RAADMA_LIVE_USER_MODULES_TBL." um , ".RAADMA_LIVE_COMPANY_TBL." c, ".RAADMA_LIVE_USER_ACCOUNTS_TBL." ua  WHERE um.user_account_fk_id = ".$users_id." and ua.company_id_fk = ".COMPANY_ID_IN_USE." and ua.company_id_fk = c.id";
			$query_executed = $db->executeQuery($query_string, $conn);
	...
	function GetListOfMonths(){
		$listOfMonths = array("01" => "January", "02" => "February","03" => "March","04" => "April","05" => "May","06" => "June","07" => "July","08" => "August","09" => "September","10" => "October","11" => "November","12" => "December");
		return $listOfMonths;
	}
   ...
   function CheckIfRecordExistsForLateMins_By_Date($emp_id){
		$bool_value = false;
		...
		$query_string = "SELECT COUNT(leaveDate) FROM ".RAADMA_LIVE_HSL_HR_EMPLOYEE_LEAVE_LOGS_TBL." WHERE employeeId = ".$emp_id."group by leaveDate";
		$exec = $db->executeQuery($query_string, $conn);
   ...
   function GetRecordIdFromLeaveData($emp_id, $leave_date){
	...
	function GetTotalEmpForDept_All_Departments(){
		$total_num_of_emp = null;

		$db = new Database();
		$db_name = HSL_DB;
		$conn = $db->connect_to_database($db_name);
		$query_string = "SELECT COUNT(*) FROM ".HSL_WEEKLY_PAID_EMP_VIEW."";	
		$exec = $db->executeQuery($query_string, $conn);
		$array_results = mssql_fetch_array($exec);
		$total_num_of_emp = $array_results[0];
	...
	function TotalNumberOfLateMinsByMonth_ByDepartment($selected_year, $selected_month){
		...
		if(isset($selected_year) && isset($selected_month)){
			...
			$query_string = 
			"SELECT Department, Sum(Num_Employees) AS 'Num_Employees', Sum(Tot_Late_Mins) AS 'Tot_Late_Mins', Sum(Tot_Late_Mins_Over_Three) AS 'Tot_Late_Mins_Over_Three' FROM
			(SELECT KBCL_W.DSCRIPTN AS 'Department', COUNT(DISTINCT KBCL_W.EMPLOYID) AS 'Num_Employees', SUM(ELL.minutesAwayFromWork) AS 'Tot_Late_Mins', 0 AS 'Tot_Late_Mins_Over_Three' FROM ".RAADMA_DB.".dbo.EMPLOYEE_LEAVE_LOGS ELL, ".HSL_DB.".dbo.KBCLWeeklyEmployees KBCL_W WHERE ELL.leaveDate >= '$firstDay' AND ELL.leaveDate <= '$lastDay' AND KBCL_W.EMPLOYID = ELL.employeeId GROUP BY KBCL_W.DSCRIPTN, KBCL_W.EMPLOYID
			UNION ALL\n".
			"SELECT KBCL_W.DSCRIPTN AS 'Department', 0 AS 'Num_Employees', 0 AS 'Tot_Late_Mins', COUNT(DISTINCT ELL.employeeId) AS 'Tot_Late_Mins_Over_Three' FROM ".RAADMA_DB.".dbo.EMPLOYEE_LEAVE_LOGS ELL, ".HSL_DB.".dbo.KBCLWeeklyEmployees KBCL_W WHERE ELL.leaveDate >= '$firstDay' AND ELL.leaveDate <= '$lastDay' AND KBCL_W.EMPLOYID = ELL.employeeId
			GROUP BY KBCL_W.DSCRIPTN, ELL.employeeId HAVING COUNT(ELL.leaveDate) >= 4 OR SUM(ELL.minutesAwayFromWork) >= 60) Late_Minutes
			GROUP BY Department;";

			$exec = $db->executeQuery($query_string, $conn);
			
			$employee_punctuality_data = $exec;
		}

		return $employee_punctuality_data;
	}


	/*function TotalNumberLateThreeOrMore_ByDepartment($selected_year, $selected_month){
		...
		if(isset($selected_year) && isset($selected_month)){
			...
			$query_string = "SELECT DSCRIPTN AS 'Department', COUNT(DISTINCT ELL.employeeId) AS 'Tot_Late_Mins' FROM ".RAADMA_DB.".dbo.EMPLOYEE_LEAVE_LOGS ELL, ".HSL_DB.".dbo.KBCLWeeklyEmployees HSL_W WHERE ELL.leaveDate >= '$firstDay' AND ELL.leaveDate <= '$lastDay' AND HSL_W.EMPLOYID = ELL.employeeId GROUP BY ELL.employeeId HAVING COUNT(ELL.leaveDate) >= 3 AND SUM(ELL.minutesAwayFromWork) >= 10, KBCL_W.DSCRIPTN";
